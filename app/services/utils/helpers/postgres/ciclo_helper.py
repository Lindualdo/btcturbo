# app/services/utils/helpers/postgres/ciclo_helper.py
...
def insert_dados_ciclo(mvrv_z: float, realized_ratio: float, puell_multiple: float, fonte: str = "Sistema") -> bool:
    """Insere novos dados no bloco ciclo"""
    try:
        logger.info(f"💾 Inserindo dados ciclo: MVRV={mvrv_z}, Realized={realized_ratio}, Puell={puell_multiple}")
        
        query = """
            INSERT INTO indicadores_ciclo (mvrv_z_score, realized_ratio, puell_multiple, fonte, timestamp)
            VALUES (%s, %s, %s, %s, %s)
        """
        params = (mvrv_z, realized_ratio, puell_multiple, fonte, datetime.utcnow())
